...
class TestIGTV:
    @pytest.fixture
    def url(self):
        return "https://www.instagram.com/tv/CIrIIMYl8VQ/"
    ...
    def page_instance(self, url):
        random_google_igtv = IGTV(url)
        random_google_igtv.scrape()
        return random_google_igtv

    def test_from_html(self, get_request, page_instance):
        igtv_html = get_request.text
        igtv_obj = IGTV(igtv_html)
        igtv_obj.scrape()
        assert igtv_obj.likes == page_instance.likes

    def test_from_soup(self, get_request, page_instance):
        igtv_html = get_request.text
        igtv_soup = BeautifulSoup(igtv_html, features='lxml')
        igtv_obj = IGTV(igtv_soup)
        igtv_obj.scrape()
        assert igtv_obj.likes == page_instance.likes
