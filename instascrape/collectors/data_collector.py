...
class IntervalCollector:
    ...
    def interval_scrape(self,
                    min_interval: int = 5, days: int=0, seconds: int=60,
                    microseconds: int=0, milliseconds: int=0, minutes: int=0,
                    hours: int=0, weeks: int=0, quiet: bool=False):
...
class IntervalIterator(IntervalCollector):
    ...
    """
    def __init__(self, scrapers, min_interval: int = 5, days: int = 0,
                        seconds: int = 60, microseconds: int = 0, milliseconds: int = 0,
                        minutes: int = 0, hours: int = 0, weeks: int = 0, quiet: bool = False):
        self.scrapers = scrapers
        ...
        if not self.quiet:
            print(f'Starting scrape, {self._calculate_time_remaining(self.current_time, self.end_time)} seconds remaining')
